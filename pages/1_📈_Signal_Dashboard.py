...
with tab1:
    ...
    # Overlay TastyTrade live price as horizontal line on chart
    if live_quote and live_quote.get("ok") and live_quote.get("last"):
        fig.add_hline(
            y=live_quote["last"],
            line=dict(color="#ffffff", width=1.5, dash="dot"),
            annotation_text=f"  LIVE ${live_quote['last']:,.2f}",
            annotation_font=dict(color="#ffffff", family="IBM Plex Mono", size=11),
            row=1, col=1,
        )
        if live_quote.get("bid"):
            fig.add_hline(y=live_quote["bid"], line=dict(color="#00ff88", width=0.8, dash="dash"), row=1, col=1)
        if live_quote.get("ask"):
            fig.add_hline(y=live_quote["ask"], line=dict(color="#ff3355", width=0.8, dash="dash"), row=1, col=1)
