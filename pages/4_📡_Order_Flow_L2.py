...
def get_session_token(username, password, is_sandbox):
...
def api_get(endpoint, token, is_sandbox):
    base = get_base_url(is_sandbox)
    try:
        resp = requests.get(
            f"{base}{endpoint}",
...
with st.spinner("Connecting to TastyTrade..."):
    session = get_session_token(cfg["username"], cfg["password"], is_sandbox)
...
st.markdown(f'<div class="connected">✅ Connected as <b>{session["user"]}</b> &nbsp;|&nbsp; {env_label} &nbsp;|&nbsp; Last updated: <b>{now_et}</b></div>', unsafe_allow_html=True)
st.markdown("---")
