...
with st.sidebar:
    ...
    ticker = st.text_input("Ticker", value="ES=F").upper()
    tf_label = st.selectbox("Timeframe", [
        "5m  — Intraday", "15m — Intraday Swing", "30m — Short Swing",
        "1h  — Swing", "1D  — Position"
    ], index=3)
    tf_map = {
        "5m  — Intraday": ("60d","5m"), "15m — Intraday Swing": ("60d","15m"),
        "30m — Short Swing": ("60d","30m"), "1h  — Swing": ("60d","60m"),
        "1D  — Position": ("6mo","1d"),
    }
...
with tab1:
    ...
    layout_opts = PLOTLY_TEMPLATE.copy()
    layout_opts["yaxis"] = dict(range=[0, 1], gridcolor="#1e2530")
    layout_opts["height"] = 300
    layout_opts["margin"] = dict(l=0, r=0, t=10, b=0)
    fig_broll.update_layout(**layout_opts)
