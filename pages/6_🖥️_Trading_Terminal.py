...
# ── Alpaca API with proper auth ────────────────────────────────────────────────
def get_alpaca_bars(symbol, timeframe):
    """Fetch real-time bars from Alpaca API with proper authentication."""
    try:
        import requests
        ...
        hist_url = f"{base_url}/v2/stocks/{symbol}/bars"
        hist_params = {
            "timeframe": alpaca_tf,
        ...
        hist_response = requests.get(hist_url, params=hist_params, auth=auth, timeout=10)
        
        if hist_response.status_code != 200:
            return None
        
        hist_data = hist_response.json()
        hist_bars = hist_data.get("bars", [])
        
        if not hist_bars:
            return None
        
        df = pd.DataFrame(hist_bars)
...
def get_yfinance_bars(symbol, timeframe):
    """Fallback: fetch bars from yfinance (delayed but reliable)."""
    tf_map = {
        "1m": ("7d", "1m"),
        "5m": ("60d", "5m"),
        "15m": ("60d", "15m"),
...
with tab_chart:
    ...
    with tc4:
        ...
        if api_key and secret_key:
            st.caption("📡 Alpaca (real-time)")
        else:
            st.caption("⏱ yfinance (delayed)")
    ...
    if df is None:
        ...
        if df is not None:
            st.info("📊 Using yfinance (Alpaca unavailable). Data is 15-20 min delayed.")
...
"""
        ...
        st.error("❌ Could not load data. Check API credentials and market hours.")
...
st.markdown("---")
st.caption(f"QuantEdge Terminal v2.4 | Lightweight Charts | {datetime.now(pytz.timezone('America/New_York')).strftime('%H:%M:%S ET')}")
