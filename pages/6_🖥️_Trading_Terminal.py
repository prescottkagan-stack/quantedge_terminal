...
# ── Alpaca Market Data API (correct endpoint) ──────────────────────────────────
def get_alpaca_bars(symbol, timeframe):
    """Fetch bars from Alpaca Market Data API (data.alpaca.markets)."""
    try:
        import requests
        ...
        params = {
            ...
            "limit": 10000,
            "feed": "sip",  # Use SIP feed for comprehensive data
        }
        
        # Only API key needed for data API
        headers = {
...
with tab_chart:
    ...
    if df is None:
        ...
        if df is not None:
            st.warning("⏱ Using yfinance (Alpaca unavailable). Check your API key in Streamlit Secrets.")
...
"""
        ...
        st.caption(f"Chart: {sym_name} • {timeframe} • {len(df)} candles • {source} • {df.index[-1].strftime('%Y-%m-%d %H:%M:%S')}")
    else:
        st.error("❌ Could not load data. Check API key and market hours.")
...
st.markdown("---")
st.caption(f"QuantEdge Terminal v2.6 | Alpaca + Lightweight Charts | {datetime.now(pytz.timezone('America/New_York')).strftime('%H:%M:%S ET')}")
