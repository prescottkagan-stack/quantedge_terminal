...
# ── Symbol definitions (TradingView format for your Pro account) ────────────
SYMBOLS = {
    "ES — S&P 500":   {"tv": "ES1!", "yf": "ES=F",  "tt": "/ES:"},
    "NQ — Nasdaq":    {"tv": "NQ1!", "yf": "NQ=F",  "tt": "/NQ:"},
    "GC — Gold":      {"tv": "GC1!", "yf": "GC=F",  "tt": "/GC:"},
    "CL — Crude":     {"tv": "CL1!", "yf": "CL=F",  "tt": "/CL:"},
    "ZB — Bonds":     {"tv": "ZB1!", "yf": "ZB=F",  "tt": "/ZB:"},
    "RTY — Russell":  {"tv": "RTY1!","yf": "RTY=F", "tt": "/RTY:"},
}
...
def tv_chart(symbol, interval, style, studies, height):
    """TradingView Lightweight Chart - pulls from your Pro account."""
...
def multi_chart(widgets_html, cols, height):
    """Wrap multiple charts in a grid."""
...
with chart_col:
    st.info("📊 **Chart loads from your TradingView Pro account** — Click the symbol button (🔍) to search ES, NQ, GC, CL, ZB, RTY or any futures/stock", icon="ℹ️")
    
    if layout == "Single":
        components.html(tv_chart(tv_sym, tv_iv, tv_style, studies_list, CHART_H), height=CHART_H, scrolling=False)
    elif layout == "Side by Side":
        h = CHART_H
        components.html(multi_chart([
            tv_inner(tv_sym,  tv_iv, tv_style, studies_list, h),
            tv_inner(tv_sym2, tv_iv, tv_style, [],           h),
        ], cols=2, height=h), height=h, scrolling=False)
    elif layout == "Stacked":
        h2 = CHART_H // 2
        components.html(multi_chart([
            tv_inner(tv_sym,  tv_iv, tv_style, studies_list, h2),
            tv_inner(tv_sym2, "D",   tv_style, [],           h2),
        ], cols=1, height=CHART_H), height=CHART_H, scrolling=False)
    else:  # Quad
        h2 = CHART_H // 2
        components.html(multi_chart([
            tv_inner(tv_sym,  tv_iv, tv_style, studies_list, h2),
        ...
        ], cols=2, height=CHART_H), height=CHART_H, scrolling=False)
