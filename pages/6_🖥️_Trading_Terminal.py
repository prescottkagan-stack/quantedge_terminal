...
def get_alpaca_bars(symbol, timeframe):
    ...
    try:
        ...
        # Construct TimeFrame correctly using TimeFrame(amount, unit)
        tf_map = {
        ...
        alpaca_tf = tf_map.get(timeframe, TimeFrame(5, TimeFrameUnit.Minute))
        ...
        request_params = StockBarsRequest(
            ...
            timeframe=alpaca_tf,  # Use string, not TimeFrame enum
            start=start_time,
            end=end_time,
            limit=10000,
            feed="iex",  # Free tier
...
st.markdown("---")
st.caption(f"QuantEdge Terminal v3.4 | Alpaca alpaca-py SDK | {datetime.now(pytz.timezone('America/New_York')).strftime('%H:%M:%S ET')}")
