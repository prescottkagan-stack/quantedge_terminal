...
def get_alpaca_bars(symbol, timeframe):
    """Fetch bars from Alpaca API using proper v2 endpoint."""
    try:
        import requests
...
with tab_chart:
    ...
    if df is None:
        ...
        if df is not None:
            st.warning("⏱ Using yfinance (Alpaca unavailable). Data is 15-20 min delayed. Check your API key & secret.")
...
st.markdown("---")
st.caption(f"QuantEdge Terminal v2.5 | Lightweight Charts | {datetime.now(pytz.timezone('America/New_York')).strftime('%H:%M:%S ET')}")
