...
def get_alpaca_bars(symbol, timeframe):
    ...
    try:
        ...
        if not api_key:
            st.error("⚠️ No Alpaca API key in secrets. Add [alpaca] api_key = '...' to .streamlit/secrets.toml")
            return None
        
        base_url = "https://data.alpaca.markets"  # Data API (includes pre-market/after-hours)
        
        url = f"{base_url}/v1beta3/trading/bars"
        ...
        if response.status_code != 200:
            st.warning(f"Alpaca API error {response.status_code}")
            return None
...
with tab_chart:
    ...
    with tc4:
        st.caption("📡 Real-time via Alpaca API")
...
"""
        ...
        st.caption(f"Chart: {sym_name} • {timeframe} • {len(df)} candles • Last: {df.index[-1].strftime('%Y-%m-%d %H:%M:%S')}")
    else:
        st.warning("⚠️ Could not load Alpaca data. Check API key and market hours.")
...
with tab_signals:
    ...
    # Load signals from Alpaca data
    @st.cache_data(ttl=30, show_spinner=False)
    def load_signals_alpaca(ticker, timeframe):
        df = get_alpaca_bars(ticker, timeframe)
    ...
    df, sig = load_signals_alpaca(sig_ticker, sig_tf)
...
st.markdown("---")
st.caption(f"QuantEdge Terminal v2.2 | Lightweight Charts + Alpaca Real-Time | {datetime.now(pytz.timezone('America/New_York')).strftime('%H:%M:%S ET')}")
