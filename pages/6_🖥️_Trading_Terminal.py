...
html, body, [data-testid="stAppViewContainer"], .main, .block-container {
    background:#060810 !important; color:#cbd5e1 !important; padding-top:4px !important;
}
...
def get_alpaca_client():
    ...
    api_key = os.environ.get("ALPACA_API_KEY") or st.secrets.get("alpaca", {}).get("api_key")
    secret_key = os.environ.get("ALPACA_SECRET_KEY") or st.secrets.get("alpaca", {}).get("secret_key")
...
def get_alpaca_bars(symbol, timeframe):
    try:
        ...
        if api is None:
            st.error("⚠️ Missing ALPACA_API_KEY / ALPACA_SECRET_KEY in Secrets")
            return None
        tf_map = {"1m":"1Min","5m":"5Min","15m":"15Min","1h":"1Hour","1D":"1Day"}
        bars = api.get_bars(symbol, tf_map.get(timeframe,"5Min"), limit=200, feed="iex").df
        if bars.empty:
            return None
...
with tab_chart:
    tc1, tc2, _, tc4 = st.columns([2,1.5,2,2])
    with tc1:
    ...
    if df is not None and not df.empty:
        ...
        candles = [{"time": int(idx.timestamp()), "open": round(float(r["Open"]),2),
                    "high": round(float(r["High"]),2), "low": round(float(r["Low"]),2),
                    "close": round(float(r["Close"]),2)} for idx, r in df.iterrows()]

        chart_html = f"""<!DOCTYPE html><html><head><meta charset="utf-8">
<style>*{{margin:0;padding:0;box-sizing:border-box}}html,body{{width:100%;height:640px;background:#060810}}
#c{{width:100%;height:100%}}.info{{position:absolute;top:10px;left:10px;color:#cbd5e1;font-size:.9em;z-index:10}}
.price{{font-size:1.2em;font-weight:bold}}.up{{color:#00e5a0}}.dn{{color:#ff2d55}}</style></head>
<body><div id="c"></div><div class="info">
<div class="price">${last_price:,.2f}</div>
<div class="{'up' if chg>=0 else 'dn'}">{chg:+.2f} ({chg_pct:+.2f}%)</div></div>
<script src="https://unpkg.com/lightweight-charts@3.8.0/dist/lightweight-charts.standalone.production.js"></script>
<script>
const chart=LightweightCharts.createChart(document.getElementById('c'),{{
  layout:{{background:{{color:'#060810'}},textColor:'#cbd5e1'}},
  grid:{{vertLines:{{color:'#1a2030'}},horzLines:{{color:'#1a2030'}}}},
  timeScale:{{timeVisible:true,secondsVisible:false}}
}});
const s=chart.addCandlestickSeries({{upColor:'#00e5a0',downColor:'#ff2d55',borderUpColor:'#00e5a0',borderDownColor:'#ff2d55',wickUpColor:'#00e5a0',wickDownColor:'#ff2d55'}});
s.setData({json.dumps(candles)});
...
window.addEventListener('resize',()=>chart.applyOptions({{width:window.innerWidth}}));
        ...
        st.caption(f"{sym_name} • {timeframe} • {len(df)} candles • IEX")
    else:
        st.info("Waiting for data...")

with tab_signals:
    st.markdown("### ⚡ QuantEdge Composite Signal")
    tc1, tc2, _ = st.columns([2,2,2])
    with tc1:
        sig_ticker = st.selectbox("Asset", list(SYMBOLS.values()), index=0, label_visibility="collapsed")
    with tc2:
    ...
    if sig and sig.get("signal") not in [None, "INSUFFICIENT_DATA"]:
        ...
        for k, v in sig.get("breakdown", {}).items():
            ...
            st.markdown(f"""<div style="display:flex;justify-content:space-between;font-family:'JetBrains Mono',monospace;font-size:0.75em;color:#4b5563;margin-bottom:2px;">
<span>{k} ({w}%)</span><span style="color:{bc};font-weight:600;">{v}</span></div>
<div class="strack"><div class="sfill" style="width:{v}%;background:{bc};opacity:0.7;"></div></div>""", unsafe_allow_html=True)
        ...
        with ic1: st.metric("RSI (14)", f"{rsi_v:.1f}")
        with ic2: st.metric("MACD", f"{macd_v:.3f}")
        with ic3: st.metric("BB %B", f"{bb_v:.2f}")
        ic4, ic5, ic6 = st.columns(3)
        with ic4: st.metric("Vol Ratio", f"{vol_r:.2f}x")
        with ic5: st.metric("Regime", regime)
        with ic6: st.metric("Ann. Vol", f"{ann_v:.1f}%")
        ic7, ic8, ic9 = st.columns(3)
        with ic7: st.metric("OLS Trend", trend)
        with ic8: st.metric("P(Bull)", f"{bayes*100:.1f}%")
        with ic9: st.metric("R²", f"{r2:.3f}")

        alerts = []
        if rsi_v > 70: alerts.append(("#ff2d55","🔴 RSI overbought"))
        if rsi_v < 30: alerts.append(("#00e5a0","🟢 RSI oversold"))
        if bb_v  > 0.9: alerts.append(("#ff2d55","🔴 Upper BB"))
        if bb_v  < 0.1: alerts.append(("#00e5a0","🟢 Lower BB"))
        if score >= 62: alerts.append(("#00e5a0","🟢 BUY signal"))
        if score <= 38: alerts.append(("#ff2d55","🔴 SELL signal"))
        if not alerts:
            st.info("✅ No alerts")
        else:
...
st.markdown("---")
st.caption(f"QuantEdge Terminal v3.5 | Alpaca IEX | {datetime.now(pytz.timezone('America/New_York')).strftime('%H:%M:%S ET')}")
